CREATE OR ALTER PROCEDURE Orders.GetOrdersByExternalId
    @OrderExternalIDs varchar(max)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    where o.OrderExternalID in (SELECT value FROM STRING_SPLIT(@OrderExternalIDs, ','));
END
GO